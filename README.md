# 🌼 Hii! I'm Myraa  💫  

### Traversed enough? — thanks for stopping by ✨ 👩‍💻💚  
Welcome to the cute little corner of my GitHub profile — grab a coffee ☕ and have a look around!

I'm passionate about technology, always eager to learn, and love building things that make a difference. Whether it's through code, collaboration, or creative problem-solving, I strive to leave a positive mark wherever I go coz productivity feel a little less boring ✨

---

## 🌷 About Me
🌱 Curious mind | Consistent learner  
🎓 CSE (AIML) undergraduate    
🎯 Currently improving **DSA, system design & production-ready apps**
✨ Lifelong learner and tech enthusiast
💡 Open to collaboration on exciting projects


---

## 🚀 What I Like Building
⚡ Real-time collaborative platforms  
🤝 Social + productivity-focused apps  
🧠 AI-powered systems with real impact  

---

## 🧩 Featured Projects

### 🎵 Emotion-Aware Music Recommendation System  
**Python | TensorFlow | OpenCV | Spotify API**  
🎧 Real-time music recommendations based on facial emotion detection  
🤖 Deep learning–based emotion classification (~65% accuracy)  
🎶 Personalized playlists mapped to user moods  

---

### 🎓 StudyNotion – Full-Stack EdTech Platform   
**React | Node.js | MongoDB | REST APIs | JWT | Payments**  
📚 Secure EdTech platform with authentication & paid courses  
💳 Integrated payment flow and progress tracking  
🛠️ Scalable backend with robust error handling  

---

### 🤝 Study Buddy – Real-Time Collaborative Study Platform  
**React | WebSockets | Firebase Auth | Gemini AI**  
⏱️ Shared Pomodoro timers & real-time chat  
👥 Friend requests, shared tasks & live sync  
🧠 AI-based focus insights, streaks & waste-time tracking  


---

## 🌱 Currently…
✨ Building real-time & AI-powered projects  
✨ Learning in public and improving every day  
✨ Connecting with like-minded creators 

---

## 📄 Resume
Want to learn more about my background, experience, and skills?
📎 Resume: *Add your link here*

---

## 📫 Let’s Connect 💌
Feel free to reach out — collaboration, learning, or just a hello 🌸  
Let’s build something amazing together!  

---

⭐ Thanks for visiting!  
C-yaa💚✨
